...
"""
# Global instance
tcgdx_fetcher = TCGdxDataFetcher()


async def sync_tcgdx_data(limit: Optional[int] = None) -> int:
    """Convenience function to sync TCGdx data"""
    async with TCGdxDataFetcher() as fetcher:
...
async def sync_single_set(set_id: str) -> bool:
    ...
    async with TCGdxDataFetcher() as fetcher:
        return await fetcher.sync_complete_set(set_id)o
import httpx
import logging
...
class TCGdxDataFetcher:
    """Service for fetching and synchronizing TCGdx Pokemon TCG data"""
